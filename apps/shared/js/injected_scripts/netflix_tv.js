// Inject Spatial Navigation in the Desktop Netflix UI.

(async function () {
  //////////////////////////// Start of spatial_navigation.js inline copy ////////////////////////////////
  ...
  console.log(`CAPYLOON SPATNAV START`);
  ...
  function getSectionNavigableElements(sectionId) {
    let res = parseSelector(_sections[sectionId].selector).filter(function (
      elem
    ) {
    ...
    if (!direction) {
      if (evt.keyCode == 13) {
        ...
        if (!currentFocusedElement) {
          if (_lastSectionId) {
            currentFocusedElement =
              getSectionLastFocusedElement(_lastSectionId);
          }
        }
  ...
  //////////////////////////// End of spatial_navigation.js inline copy ////////////////////////////////

  try {
    console.log("CAPYLOON: Netflix TV Mode");

    SpatialNavigation.init();
    // Define navigable elements
    SpatialNavigation.add("items", { selector: ".slider-item" });

    // Make the navigation tab items focusable.
    document.querySelectorAll("li.navigation-tab").forEach((item) => {
      item.tabIndex = 0;
    });

    SpatialNavigation.add("navigation-tab", {
      selector: "li.navigation-tab",
    });
    SpatialNavigation.add("secondary-navigation-tab", {
      selector: ".secondary-navigation div",
    });
    SpatialNavigation.add("buttons", { selector: "button" });

    // Make the *currently existing* navigable elements focusable.
    SpatialNavigation.makeFocusable("navigation-tab");
    // Focus the first navigable element.
    SpatialNavigation.focus();
  } catch (e) {
    console.error(`CAPYLOON: oops ${e}`);
  }
